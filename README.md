...
**在线使用：<https://ssllf8.github.io/jobpipe/>** —— 手机打开后按页面提示「装到手机」，即为可离线使用的全屏 App。
...
**自荐信进不进公开版：2026-09-22 起改为"进"。**
正文点名了投递的公司（"应聘辽宁鑫锡的……岗位"），原先怕暴露"在投哪些公司"而一律排除，
代价是**手机上看不到正文** —— 可投递恰恰要在手机上复制发出去。
而且 CI 是从仓库 clone 后构建的：`letters.json` 不在仓库里，CI 就永远读不到，
线上会一直显示"还没生成"。权衡后改为默认内联。
要出一份不带自荐信的干净版（例如发给招聘方看），加 `--no-letters`。
...
地址：**<https://ssllf8.github.io/jobpipe/>**（必须是 https，`file://` 无法注册 Service Worker）

**先别看表 —— 页面自己会给入口。** 只要浏览器允许安装，顶栏会多出一个下载图标，
同时在顶栏下方弹一条「装到手机，像 App 一样全屏打开」：点「安装」直接弹系统安装框。
点 ✕ 忽略之后提示条不再出现（记在 `localStorage`），但顶栏那个图标还在。

为什么把入口搬进页面里：**站点「可安装」和用户「装得上」是两件事。**
本站在 Chrome 眼里完全合格 —— 安装资格接口（`Page.getInstallabilityErrors`）返回零错误、
manifest 解析无误、图标齐全、Service Worker 正常接管；但入口长在浏览器菜单里，
而 Chrome 要等它自己认定用户确实在用这个站，才肯把「安装应用」放进菜单。
国产 ROM 的定制浏览器、微信内置浏览器则根本没有这一项。所以由应用接管
`beforeinstallprompt`，自己决定入口放在哪、什么时候出现。

拿不到那个入口时（iOS Safari、微信里打开、或 Chrome 还没放开），按下表走：

| 系统 | 浏览器 | 操作 |
|---|---|---|
| Android | Chrome / Edge | 「⋮」→ **「安装应用」**（部分版本叫「添加到主屏幕」）→ 确认 |
| iOS / iPadOS | **Safari**（必须） | 底部「分享」→ **「添加到主屏幕」** → 添加 |

在应用内也能查：右上角「⋯」→「数据与设置」→ **装到手机** ——
这一段会按你的设备（Android / iPhone / 微信 / 桌面）给出对应路径，
已经装成 App 打开时则改为告诉你「已经在以 App 方式运行」。

装完的表现：桌面多一个深色「职」字图标，点开**全屏无地址栏**，
断网也能打开（Service Worker 预缓存了整页）。

四个容易踩的点：

1. **iOS 只能用 Safari**。iOS 上所有浏览器的内核都是 WebKit，但
   「添加到主屏幕」只挂在 Safari 的分享菜单里 —— 用 Chrome 打开是找不到这个入口的。
2. **微信内置浏览器装不了**。微信里点开只能浏览，没有安装入口；
   点右上角「···」→「在浏览器中打开」，再按上表操作。所以别把链接直接发微信里点。
3. **Chrome 菜单里暂时没有「安装应用」**：在页面上多停留十几秒、点几下再打开菜单 ——
   Chrome 要先确认你确实在用它才肯给入口。另外若以前点过一次「添加到主屏幕」，
   主屏上可能已经有图标了。
4. **更新是自动的，但需要一次刷新**。Service Worker 在后台发现新版本会弹一条
   「有新版本，立即刷新」，点一下即可；不点就下次进页面时生效。
   这不是缺陷而是刻意的 —— 免得你正看着页面，它自己刷掉了。
...
# 安装入口验证（入口出现 → 接受 → 拒绝 → 已装成 App 时不再引导）
python tools/e2e_install.py
```
...
或者部署 `index.html` 后在手机浏览器里打开，按页面提示「装到手机」即为全屏 App，且断网可用。
...
│   └── e2e_install.py  Playwright 安装入口验证（顺带向 Chrome 要一次安装资格结论）
├── .github/workflows/deploy.yml   测试 → 构建 → 审计 → 部署 Pages
└── tests/              166 项测试（conftest.py 把种子钉在初始那份，采集不干扰测试）
```
...
python tools/e2e_install.py  # 25 项断言（入口出现 → 接受 → 拒绝 → 已安装）
```
...
- **自荐信层** —— 提示词护栏条款仍在、占位符全部填充、`letters.json` 坏结构被跳过、
  只挑 S 级且跳过已生成、**真实自荐信必须真的进了公开产物**（回归防线，2026-09-22 方向反转）
- **PWA 层** —— manifest 字段与路径、图标声明尺寸**与 PNG 实际像素逐一对齐**、
  SW 处理器齐全、不在 `install` 阶段抢跑、导航走 network-first、
  只有可发布产物带 PWA 资产、版本号随内容变、发布闸门各类泄漏都能拦下、
  **应用内安装入口真的接管了 `beforeinstallprompt`**（而不是让浏览器自己弹那条迷你提示）、
  已装成 App 时不再引导安装、三个平台的手动步骤文案没串
...
`tools/e2e_install.py` 是 M7 加的一环。它要回答的是一句用户反馈：
「菜单中没有安装应用」。查下来站点本身合格 —— 于是它顺手把 Chrome 的
**安装资格结论**（CDP `Page.getInstallabilityErrors`）也断言了进去，
剩下的力气全花在验证应用内那套接管逻辑上：
...
- 自荐信正文点名了投递的公司 —— 2026-09-22 起**主动公开**（手机上要复制投递），
  `data/letters.json` 随之入库；要出干净版用 `build.py --no-letters`
- `data/state.json`、`data/exports/`、`.env`、`dist/`、`dist-publish/` 均在 `.gitignore` 中
...
2. `state_inlined` 必须为 false、`state_source` 不得是 `file:` —— 投递记录绝不能进产物
3. 不得出现 API key、疑似本机绝对路径
4. manifest 路径必须相对、图标必须真实存在、`sw.js` 版本号必须已替换

> 关于自荐信：它本身就是要发给 HR 看的，不含私密信息。公开它的代价是暴露"在投哪几家"，
> 换来的是手机上能直接复制投递 —— 这是明确权衡后的选择，想收回就用 `--no-letters`。

## 路线图

- [x] **M1** 数据模型 + 打分引擎 + 构建脚本 + 手机端只读渲染
- [x] **M2** 状态流转 + 跟进计划 + 备注与时间线 + 手动新增 + 导入导出
- [x] **M3** 自荐信生成（DeepSeek）+ 缓存 + 一键复制（2026-09-22 起公开产物内联）
...
- [x] **M7** 应用内安装入口（接管 `beforeinstallprompt`：提示条 + 顶栏按钮 + 分平台手动步骤）
      —— 起因是一句真实反馈「菜单中没有安装应用」，查完发现站点合格、是入口不可达
